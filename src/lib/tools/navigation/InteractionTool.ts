...
export class InteractionTool {
  ...
  private async _inputTextElement(description: string, text: string): Promise<ToolOutput> {
    for (let attempt = 1; attempt <= NUM_RETRIES; attempt++) {
      try {
        ...
        // Emit status message
        this.executionContext.getPubSub().publishMessage(PubSub.createMessage(`⌨️ Typed "${text}" into ${description}`, 'thinking'))
  ...
  private async _clearElement(description: string): Promise<ToolOutput> {
    for (let attempt = 1; attempt <= NUM_RETRIES; attempt++) {
      try {
        ...
        // Emit status message
        this.executionContext.getPubSub().publishMessage(PubSub.createMessage(`🗑️ Cleared: ${description}`, 'thinking'))
  ...
  private async _sendKeys(keys: string): Promise<ToolOutput> {
    ...
    // Emit status message
    this.executionContext.getPubSub().publishMessage(PubSub.createMessage(`⌨️ Sent keys: ${keys}`, 'thinking'))
